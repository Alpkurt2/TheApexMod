...
{
    public class GalaxyBlast : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                {
                    int proj = Projectile.NewProjectile(spawn, speed, ModContent.ProjectileType<GalaxyBullet>(), damage, 1f, Main.myPlayer);
                    Main.projectile[proj].tileCollide = false;
                }
                else
                {
                    int proj1 = Projectile.NewProjectile(spawn1, speed1, ModContent.ProjectileType<GalaxyBullet>(), damage, 1f, Main.myPlayer);
