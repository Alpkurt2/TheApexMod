...
{
	public class NoxBeam : ModProjectile
	{
		private const float MAX_CHARGE = 25f;
		private const float MOVE_DISTANCE = 150f;
		...
		public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
		{
			...
			for (float i = transDist; i <= Distance; i += step)
			{
				...
				spriteBatch.Draw(texture, origin - Main.screenPosition,
					new Rectangle(0, 26, 28, 20), i < transDist ? Color.Transparent : c, r,
					new Vector2(28 * .5f, 20 * .5f), 3f, 0, 0);
			}
			spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition,
				new Rectangle(0, 0, 28, 26), Color.White, r, new Vector2(28 * .5f, 26 * .5f), 3f, 0, 0);
			spriteBatch.Draw(texture, start + (Distance + step) * unit - Main.screenPosition,
				new Rectangle(0, 52, 28, 26), Color.White, r, new Vector2(28 * .5f, 26 * .5f), 3f, 0, 0);
		}
		...
		private void SpawnDusts(Player player)
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos, 0, 0, 135, dustVel.X, dustVel.Y)];
				dust.noGravity = true;
				dust.scale = 5f;
			}
		}
